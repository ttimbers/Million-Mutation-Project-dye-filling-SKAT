...
main <- function(){
  ...
  ## rename columns
  colnames(SKAT_results) <- c("sequence", "p-value", "number_of_variants", "q-value")
  colnames(effectsize) <- c("sequence", "number_of_variants", "number_of_strains_with_dyf_phenotype", "number_of_strains_with_wild-type_phenotype", "effects_size")
  
  ## reorder columns 
  SKAT_results <- SKAT_results[c("sequence", "p-value", "q-value", "number_of_variants")]

  ## join SKAT_results and effectsize
  joined_data <- left_join(SKAT_results, effectsize)
