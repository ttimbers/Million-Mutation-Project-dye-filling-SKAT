...
main <- function() {
  ...
  weights.file <- args[5]
	...
	## perform SKAT on all sets of variants (use weights)
	set.seed(1234)
	...
	p.values.weights$p_adjust <- p.adjust(p.values.weights$P.value, method = "BH")
	write.table(p.values.weights, paste(path_to_plink_files, "/SKAT_weights.tsv", sep=""), sep="\t", row.names=FALSE, quote=FALSE, append=FALSE)
