...
custom_weights <- function(filename, weight.KO, weight.inframe.indel, weight.missense) {
  ...
  system("grep -h 'INDEL' MMPdyf_non-syn_coding_no_header.txt > MMPdyf_indels.txt")
  system("grep -v 'INDEL' MMPdyf_non-syn_coding_no_header.txt > MMPdyf_snvs.txt")

  ##import text files created above
  variants.indels <- read.table("MMPdyf_indels.txt")
  variants.snvs <- read.table("MMPdyf_snvs.txt")
